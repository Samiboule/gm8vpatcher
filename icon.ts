...
export const SaveIcon = async function(iconData: Array<WindowsIcon>, dir: string): Promise<void> {
	await fs.remove(dir);
	await fs.mkdir(dir);
...
const WriteArray = function(buff: SmartBuffer, arr: Array<number>): void {
	...
	switch(bpp){
		...
		case 8:
			...
			for(let i: number = 0; i < pixelCount; ++i){
				...
				WriteArray(bgraData, blob.slice(lutPos, lutPos+4));
			}
			let cursor: number = 0;
